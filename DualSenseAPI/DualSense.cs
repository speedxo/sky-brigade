...
{
    /// <summary>
    /// A utillity class to simplyify the original codebase.
    /// </summary>
    public class UnderlyingDevice : IDisposable
    {
        public HidDevice Device { get; private init; }
        public HidStream? Stream { get; private set; }
        public bool HasBeenOpened { get; private set; }

        public UnderlyingDevice(HidDevice device)
        {
            this.Device = device;
        }
    ...
    public class DualSense
    {
        ...
        private DualSense(HidDevice underlyingDevice, int? readBufferSize, int? writeBufferSize)
        {
            this.underlyingDevice = new UnderlyingDevice(underlyingDevice);
        ...
        private async Task<DualSenseInputState> ReadWriteOnceAsync()
        {
            ...
            await underlyingDevice.Stream!.WriteAsync(GetOutputDataBytes());

            byte[] bytes = new byte[readBufferSize!.Value];
            await underlyingDevice.Stream!.ReadExactlyAsync(new Memory<byte>(bytes));

            // this can effectively determine which input packet you've recieved, USB or bluetooth, and offset by the right amount
            int offset = IoMode switch
            {
                IoMode.USB => 1, // USB packet flag
                IoMode.Bluetooth => 2, // Bluetooth packet flag
                _ => 0
            };
